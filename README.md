...
Each numbered directory (`0001`, `0002`, ...) corresponds to one model from the parameter grid used in the paper.

Each directory contains:

- Synthetic spectra generated with **RADMC-3D**
- The corresponding **RADMC-3D** input files required to reproduce the simulation

Users may rerun the same model with different numerical settings (e.g., wavelength sampling or photon statistics) to generate spectra with different accuracies or spectral resolutions.
...
- the spectral fitting model (Fig.6 of the related paper),
- the complete parameter grid,
- additional model visualizations (e.g., surface density and temperature structures),
- and other products that facilitate the interpretation and fitting of circumplanetary disk spectra.
